...
@router.get("/", response_model=list[AnalyticsPerRangeOfDates | None])
async def get_analytics_by_date(date_start: date = Query(...), date_end: date = Query(...), db=Depends(get_db)):
    result = await get_stats_by_range_of_date(db=db, start_date=date_start, end_date=date_end)
